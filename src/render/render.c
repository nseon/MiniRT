...
/*   Updated: 2025/08/08 21:42:01 by nseon            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (++x < img->w)
	{
		...
		while (++y < img->h)
		{
			...
			if (nb_ray <= 1)
				...
				vp = win_to_vp(gctx, x + frandom(random, 0, 1), y + frandom(random, 0, 1), img);
			...
			// ren.d.y += nb_ray;
			ren.o = gctx->cam.pos;
			add_rgb96_t(&gctx->color_px[x * WIN_H + y], trace_ray(gctx, ren, 0, random));
