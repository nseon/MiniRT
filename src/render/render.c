...
void	compute_cam_matrice(t_camera *cam)
{
	...
	set_cam_transform(cam, mx_rotation_y(cam->y_rot,
			rotation_x(cam->x_rot, buf)));
	if (dabs(tp_dot(vector(0, 1, 0), cam->orient)) > 0.95)
		mul_cam_transform(cam, mtx4_view(cam->pos,
				tp_add(cam->pos, cam->orient), vector(0, 0, 1), buf));
	else
		mul_cam_transform(cam, mtx4_view(cam->pos,
				tp_add(cam->pos, cam->orient), vector(0, 1, 0), buf));
}
